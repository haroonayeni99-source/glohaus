...
if (!process.env.MIGRATION_DATABASE_URL || !authId?.startsWith("user_") || !operator || !reason || reason.length < 5) {
  console.error('Usage: pnpm admin:grant user_ID operator-reference "Reason for access". Requires MIGRATION_DATABASE_URL.');
  process.exit(1);
}
